...
<body class=" flex flex-col justify-center items-center min-h-screen">
    ...
    <div class="flex flex-col md:flex-row justify-center space-y-4 md:space-y-0 md:space-x-4 w-full px-4">
        <a href="#sel" class="bg-gray-200 text-lg md:text-2xl text-gray-800 py-6 md:py-12 px-4 md:px-16 rounded-xl font-semibold md:mr-16 border-gray-200 text-center">Создать товар с помощью формы</a>
        <a href="{{route('fromlist')}}#sel" class="bg-gray-200 text-gray-800 text-lg md:text-2xl py-6 md:py-12 px-4 md:px-16 rounded-xl font-semibold text-center">Загрузить товары из прайс-листа</a>
    </div>
</div>

<!-- Основной контейнер с использованием Grid -->
<div class="grid grid-cols-1 md:grid-cols-[1fr_2fr_1fr] gap-8 w-full max-w-6xl mb-20 px-4 md:px-0">
    
    <!-- Левая пустая колонка (скрыта на мобильных) -->
    <div id="sel" class="hidden md:block"></div>

    <!-- Центральная колонка (форма) -->
    <div id="create-form" class="bg-white p-6 md:p-8 rounded-lg mt-10 md:mt-20">
        <h1 class="text-center text-lg md:text-xl font-semibold mb-6">Создать товар с помощью формы</h1>
        ...
        <form action="{{ route('adverts.store') }}" method="POST">
            ...
            <div class="mb-4">
                ...
                <select id="brand" name="brand" data-url="{{ route('get.modelsCreate') }}" class="w-full px-3 py-2 border rounded-lg">
            ...
            <div class="mb-4">
                <label for="branch_id" class="block text-gray-700 font-medium mb-2">Филиал <span class="text-red-500">*</span></label>
                <select id="branch_id" name="branch_id" class="w-full px-3 py-2 border rounded-lg" required>
    ...
    <!-- Правая колонка (ссылка на импорт товаров, скрыта на мобильных) -->
    <div class="hidden md:block bg-white p-4 rounded-lg shadow-md self-start">

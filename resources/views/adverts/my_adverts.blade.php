...
<!-- /Yandex.Metrika counter -->
    ...
    .table-row {
    cursor: pointer; /* Изменяет курсор на "палец" при наведении */
...
<div class="grid grid-cols-1 md:grid-cols-10 gap-2 items-center w-full bg-gray-100 mb-4">
    <form method="GET" action="{{ route('adverts.my_adverts') }}" class="border-r border-gray-300 w-full h-auto md:h-10 p-1 pl-4 search-form flex flex-col md:flex-row items-start md:items-center justify-start gap-1 col-span-1 md:col-span-5">
        <input type="text" name="search" class="h-10 md:h-full searchInput p-1 border rounded-md w-full md:w-5/5 text-sm" placeholder="Поиск по наименованию или номеру" value="{{ request()->input('search', $search ?? '') }}">

<select name="brand" class="brandFilter h-10 md:h-full p-1 border md:w-2/5 md:w-3/10 rounded-md text-sm">
    <option value="" selected>Все марки</option>
    <option value="__empty__">Не указано</option>
    @foreach($brands as $brand)
        @if(!is_null($brand) && trim($brand) !== '')
            <option value="{{ $brand }}" {{ request('brand') == $brand ? 'selected' : '' }}>{{ $brand }}</option>
        @endif
    @endforeach
</select>

        <button type="submit" class="btn-search p-2 text-xs bg-blue-500 text-white rounded-md w-full md:w-auto">Поиск</button>
        <a href="{{ route('adverts.my_adverts') }}" class="text-center btn-reset p-2 text-xs bg-gray-300 text-black rounded-md w-full md:w-auto">Сбросить</a>
    </form>

    <!-- Блок с кнопками "Удалить несколько" и "Удалить выбранные" -->
    <div class="border-r border-gray-300 w-full h-auto md:h-10 gap-1 col-span-1 md:col-span-3 flex flex-col md:flex-row items-start md:items-center justify-center">
        <button id="deleteMultipleBtn" class="w-full md:w-1/4 text-xs btn-delete-multiple p-2 bg-gray-500 text-white rounded-md">Выбрать</button>
        <button id="deleteSelectedBtn" class="w-full md:w-2/5 text-xs btn-delete-selected p-2 bg-gray-300 text-gray-500 rounded-md opacity-100 cursor-not-allowed whitespace-nowrap" disabled>Удалить выбранные</button>
    </div>

<!-- Выпадающий список статусов -->
<div class="col-span-1 md:col-span-2 flex items-center justify-center p-1">
    <span class="mr-2 text-xs">Отображать:</span>
    <select id="statusFilter" name="status" class="appearance-none rounded p-2 text-sm font-bold bg-gray-100 focus:outline-none focus:ring-0">
        <option value="activ" {{ request()->get('status', $statusFilter ?? '') == 'activ' ? 'selected' : '' }}>Активные</option>
        <option value="not_activ" {{ request()->get('status', $statusFilter ?? '') == 'not_activ' ? 'selected' : '' }}>Неактивные</option>
        <option value="arhiv" {{ request()->get('status', $statusFilter ?? '') == 'arhiv' ? 'selected' : '' }}>Архивные</option>
        <option value="sale" {{ request()->get('status', $statusFilter ?? '') == 'sale' ? 'selected' : '' }}>Проданные</option>
        <option value="all" {{ request()->get('status', $statusFilter ?? '') == 'all' ? 'selected' : '' }}>Все статусы</option>
    </select>
</div>

</div>

@if ($adverts->isNotEmpty() && $adverts->contains('status_pay', 'limited'))
    <div class="bg-orange-100 px-4 py-2 rounded relative mb-4" role="alert">
        <span class="block sm:inline">Вы добавили больше товаров, чем в Вашем тарифе. Товары сверх тарифа неактивны - покупатели их не увидят, неактивные товары помечены в таблице красным цветом. Увеличьте количество товаров в <a href="{{ route('tariff.settings') }}" class="text-blue-500 hover:underline">настройках тарифа</a>.</span>
    </div>
@endif
...
@if ($adverts->isEmpty())
    <p>У вас нет товаров с выбранными критериями.</p>
@else
        <div class="overflow-x-auto w-full px-4">
            <table class="table w-full border-collapse pointer">
                <thead >
                    <tr class="bg-gray-100">
                        <th class=" border p-0">Артикул</th>
                        <th class=" border p-0">Наименование</th>
                        <th class=" border p-0">Состояние</th>
                        <th class=" border p-0">Марка</th>
                        <th class=" border p-0">Модель</th>
                        <th class=" border p-0">Кузов</th>
                        <th class=" border p-0">Номер</th>
                        <th class=" border p-0">Двигатель</th>
                        <th class=" border p-0">Год</th>
                        <th class=" border p-0">L/R</th>
                        <th class=" border p-0">F/R</th>
                        <th class=" border p-0">U/D</th>
                        <th class=" border p-0">Цена</th>
                        <th class=" border p-0">Цвет</th>
                        <th class=" border p-0">Применимость/Описание</th>
                        <th class=" border p-0">Количество</th>
                        <th class=" border p-0">Наличие</th>
                        <th class=" border p-0">Действия</th> <!-- Новый столбец для кнопок -->
                    </tr>
                </thead>
                <tbody>
                 @foreach($adverts as $advert)
   <tr class="table-row {{ $advert->status_pay == 'limited' ? 'bg-red-100' : '' }}"
    data-id-info="{{ $advert->id }}"
    ...
    data-status-ad="{{ $advert->status_ad }}"
    data-new-used="{{ $advert->new_used }}"
    data-l-r="{{ $advert->L_R }}"
    data-f-r="{{ $advert->F_R }}"
    data-u-d="{{ $advert->U_D }}"
    data-color="{{ $advert->color }}"
    data-applicability="{{ $advert->applicability }}"
    data-quantity="{{ $advert->quantity }}"
    data-availability="{{ $advert->availability }}">
...
<footer class="bg-white text-white py-20 shadow-none text-center mt-20 w-full hidden md:block">
    ...
    <option value="Сверху"  selected >Сверху</option>
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // Обработчик изменения выпадающего списка статусов
    ...
    if (table) {
        ...
        const hasTableHeadRow = thead && thead.querySelector('tr');
        let isMultipleDeleteMode = false;
        const hasDeleteMultipleBtn = !!deleteMultipleBtn; // Преобразуем в boolean
        const hasDeleteSelectedBtn = !!deleteSelectedBtn; // Преобразуем в boolean


        // Обработчик клика на кнопку "Выбрать"
        if (hasDeleteMultipleBtn) {
       ...
       // Обработчик клика на строку (только если есть tbody)
        if (tbody) {
            tbody.querySelectorAll('tr').forEach(row => {
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    document.querySelectorAll('.edit-btn').forEach(button => {
        button.addEventListener('click', (event) => {
            ...
            const row = button.closest('tr'); // Получаем строку таблицы
            const modal = document.getElementById('editModal'); // Получаем модальное окно

            // Заполняем поля модального окна данными из строки таблицы
            document.getElementById('editAdvertId').value = row.dataset.idInfo;
            document.getElementById('art_number').value = row.dataset.artNumber;
            document.getElementById('product_name').value = row.dataset.productName;
            document.getElementById('number').value = row.dataset.numberInfo;
            document.getElementById('new_used').value = row.dataset.newUsed;
            document.getElementById('brand').value = row.dataset.brandInfo;
            document.getElementById('model').value = row.dataset.modelInfo;
            document.getElementById('year').value = row.dataset.year;
            document.getElementById('body').value = row.dataset.bodyInfo;
            document.getElementById('engine').value = row.dataset.engineInfo;
            document.getElementById('color').value = row.dataset.color;
            document.getElementById('applicability').value = row.dataset.applicability;
            document.getElementById('quantity').value = row.dataset.quantity;
            document.getElementById('price').value = row.dataset.priceInfo;
            document.getElementById('availability').value = row.dataset.availability;
            document.getElementById('main_photo_url').value = row.dataset.mainPhotoUrl;
            document.getElementById('additional_photo_url_1').value = row.dataset.additionalPhotoUrl1;
            document.getElementById('additional_photo_url_2').value = row.dataset.additionalPhotoUrl2;
            document.getElementById('additional_photo_url_3').value = row.dataset.additionalPhotoUrl3;

            // Устанавливаем значения для выпадающих списков
            document.getElementById('L_R').value = row.dataset.lR; // Слева/Справа
            document.getElementById('F_R').value = row.dataset.fR; // Спереди/Сзади
            document.getElementById('U_D').value = row.dataset.uD; // Сверху/Снизу
    ...
    // Закрытие модального окна при клике вне их
    window.addEventListener('click', (event) => {
        const modals = document.querySelectorAll('.modal');
        modals.forEach(modal => {
            if (event.target === modal) {

...
<script>
  ...
  passwordInput.addEventListener('input', () => {
    ...
    const hasLatinOnly = /^[A-Za-z0-9!@#$%^&*()_+\-=\[\]{};':"\\|,.<>\/?]*$/.test(password);

    if (!hasLatinOnly) {
      passwordMessage.textContent = 'Пароль должен содержать только латинские символы';
      passwordMessage.classList.remove('text-green-500');
      passwordMessage.classList.add('text-red-500');
    } else if (!hasLength || !hasUpperCase) {
      passwordMessage.textContent = 'Пароль должен содержать минимум 8 символов и одну заглавную букву';
      passwordMessage.classList.remove('text-green-500');
      passwordMessage.classList.add('text-red-500');
    } else {
      passwordMessage.textContent = 'Пароль соответствует требованиям';
  ...
  function togglePasswordVisibility(passwordId, confirmPasswordId, element) {
    ...
    if (passwordInput.type === 'password') {
      ...
      passwordIcon.src = "images/open_password.png"; // Замените на путь к открытому глазу
      passwordIcon.alt = "Скрыть";
    } else {
      passwordInput.type = 'password';
      confirmPasswordInput.type = 'password';
      passwordIcon.src = "images/close_password.png"; // Замените на путь к закрытому глазу

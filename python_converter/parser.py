...
def open_file(file, filename, skip_rows, encoding, delimiter, nrows=None):
    data = 0
    is_excel = False
    ...
    if extension == 'csv' or extension == 'txt':
        ...
        data = pd.read_csv(file, encoding=encoding, delimiter=delimiter, skiprows=skip_rows, nrows=nrows)

    elif extension == 'json':
        data = pd.read_json(file,nrows=nrows)
    elif extension == 'xml':
        ...
        for sheet in sheet_names:
            df = excel_file.parse(sheet, skiprows=skip_rows, nrows=nrows)
            for sheet1 in data:
    ...
    return data, is_excel


def parse_file_to_df(file, filename, user_id: int, id_filial: int, column_names: dict, skip_rows, encoding, delimiter,
    ...
    data, is_excel = open_file(file, filename, skip_rows, encoding, delimiter)
    ...
    if is_excel:
        for sheet in data:
            ...
            if add_sheet_name_to_product_name:
                add_name = sheet
            df = format_df_for_db(data[sheet], user_id, id_filial, column_names, add_name, extract_data_from_product_name, skip_empty_price_rows, split_symbols, selected_brands, remove_duplicates)
            result_df = pd.concat([result_df, df], ignore_index=True)

    else:
        # if not excel
        result_df = format_df_for_db(data, user_id, id_filial, column_names, "", extract_data_from_product_name, skip_empty_price_rows, split_symbols, selected_brands, remove_duplicates)

    return result_df


def format_df_for_db(data: pd.DataFrame, user_id: int, id_filial: int, column_names: dict, add_to_product_name: str, extract_data_from_product_name: bool, skip_empty_price_rows: bool, split_symbols:str, selected_brands: list, remove_duplicates: bool):
    right_columns = ['user_id', 'art_number', 'product_name', 'new_used', 'brand', 'model', 'body', 'number', 'engine',
               ...
               'additional_photo_url_3', 'id_branch']
    ...
    result_df['id_branch'] = int(id_filial)
    ...
    result_df['created_at'] = pd.to_datetime('now')

...
from parser import parse_file_to_df, open_file
...
async def get_status(task_id: int):
    """
        ...
        Возвращает один из статусов: 'waiting', 'in_progress', 'done', 'error'
    """
    status = get_status_by_task_id(task_id)
    if status:
        return {"task_id": task_id, "status": status["status"], "lines_added": status["lines_added"], "error_msg": status["error_msg"]}
    raise HTTPException(status_code=404, detail="Task not found")

@app.post("/get_columns")
async def get_columns(file: UploadFile = File(...), skip_rows: int = 0, csv_encoding: str = "auto", csv_delimiter: str = ","):
    file_stream = BytesIO(await file.read())
    try:
        df, is_excel = open_file(file_stream, file.filename, skip_rows, csv_encoding, csv_delimiter, 0)
...
@app.post("/upload")
async def upload_data(user_id: int, columns: str, background_tasks: BackgroundTasks, id_filial:int, file: UploadFile = File(...), skip_rows: int = 0, csv_encoding: str = "auto", csv_delimiter: str = ",", add_sheet_name_to_product_name: bool = True, extract_data_from_product_name: bool = True, skip_empty_price_rows:bool = True, deactivate_old_ad:bool = True, split_symbols:str = " ", selected_brands: str = "", remove_duplicates:bool = True):
    """
       Отправляет файл в очередь на обработку.

       Параметры:
       - `user_id` (int): Идентификатор пользователя, от имени которого выполняется загрузка.
       - `id_filial` (int): Идентификатор филиала, для которого выполняется загрузка. (id_branch в БД)
       ...
       - `deactivate_old_ad` (bool, по умолчанию True): Выставить ли для старых объявлений статус 'arhiv'
       ...
       - `remove_duplicates` (bool, по умолчанию True): Удалять ли дубликаты строк.
       """
    try:
    ...
    background_tasks.add_task(process_file, task_id, user_id, id_filial, columns, file_stream, file.filename, skip_rows, csv_encoding, csv_delimiter,
                              add_sheet_name_to_product_name, extract_data_from_product_name, skip_empty_price_rows, deactivate_old_ad, split_symbols, selected_brands, remove_duplicates)

    return {"message": "File added to the queue", "task_id": task_id, "queue_position": queue_position}

def process_file(task_id:int, user_id: int, id_filial: int, columns: dict, file_stream, filename, skip_rows: int, encoding: str, delimiter: str, add_sheet_name_to_product_name: bool, extract_data_from_product_name: bool, skip_empty_price_rows, deactivate_old_ad, split_symbols, selected_brands, remove_duplicates):
    ...
    try:
        update_task_status(task_id, "in_progress")
        df = parse_file_to_df(file_stream, filename, user_id, id_filial, columns, skip_rows, encoding, delimiter,
                              add_sheet_name_to_product_name, extract_data_from_product_name, skip_empty_price_rows, split_symbols, selected_brands, remove_duplicates)
    except Exception as e:
        error_msg = f"Error while parsing file {filename}: {type(e).__name__} {str(e)}"
        update_task_status(task_id, "error", error_msg=error_msg)
        log(error_msg)
        return
    ...
    try:
        lines_added = 0
        if df is not None:
            lines_added = add_dataframe_to_db(df, user_id, deactivate_old_ad)
        update_task_status(task_id, "done", lines_added=lines_added)
    except Exception as e:
